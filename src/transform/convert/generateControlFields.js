...
export function handle007(marcRecord, record) {
  if (marcRecord.get(/^007$/u).length === 0) { // eslint-disable-line functional/no-conditional-statements
    const fields = createMaterialFields(record) || [];

    fields.forEach(f => {
      if (f.tag === '008') { // eslint-disable-line functional/no-conditional-statements
        const [f008] = marcRecord.get(/^008$/u);
        f008.value = f.value; // eslint-disable-line functional/immutable-data
      } else if (f.tag === '007') { // eslint-disable-line functional/no-conditional-statements
        marcRecord.insertField(f);
      } else if (f.tag === '006') { // eslint-disable-line functional/no-conditional-statements
        const [f006] = marcRecord.get(/^006$/u);

        if (f006) { // eslint-disable-line functional/no-conditional-statements
...
export function handle008(marcRecord, testRun = false) {
  ...
  if (f008) { // eslint-disable-line functional/no-conditional-statements
    ...
    // if (chars[17] === ' ') { // eslint-disable-line functional/no-conditional-statements
    ...
    if (['#', '^', 'd', 'u', '|', ' ', ''].includes(chars[39])) { // eslint-disable-line functional/no-conditional-statements
      chars[39] = chars[39] === '' ? '|' : 'c'; // eslint-disable-line functional/immutable-data
    }

    if (marcRecord.leader[6] === 'r' && chars[33] === 'g') { // eslint-disable-line functional/no-conditional-statements
